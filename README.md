...
## 5. メール認証(Resend)のセットアップ

学生登録時に確認メールを送信し、メール内のリンクをクリックするまでログインできない仕組みを追加しました。

1. https://resend.com でアカウント作成(クレジットカード不要、無料枠は月3,000通・1日100通まで)
2. ダッシュボードの **API Keys** から新しいキーを発行してコピー
3. Renderの `gakushoku-api` → **Environment** タブで以下を追加
   - `RESEND_API_KEY`: 発行したAPIキー
   - `APP_BASE_URL`: RenderのバックエンドURL(例: `https://gakushoku-api.onrender.com`)。確認メール内のリンク生成に使用します
   - `FROM_EMAIL`: 送信元アドレス。独自ドメインを検証するまでは既定値の `onboarding@resend.dev` のままでOK

### ⚠️ ドメイン未検証の間の制限

`onboarding@resend.dev` を送信元にしている間は、**Resendアカウント作成時に使ったメールアドレス宛にしか送信できません**(なりすまし送信防止のための仕様)。学生一人ひとりの実際のメールアドレスに送るには、独自ドメインの取得とResendでの検証(DNSにTXT/CNAMEレコードを追加)が必要です。

ドメインを取得したら:
1. Resendダッシュボードの **Domains** から追加し、案内されたDNSレコードをドメインの管理画面(お名前.com、Cloudflare、Google Domainsなど)に設定
2. 検証が完了したら、`FROM_EMAIL` を `noreply@あなたのドメイン` などに変更

### RESEND_API_KEY未設定の場合

`RESEND_API_KEY` を設定しなくても登録自体は動作します。この場合メールは送信されず、代わりにRenderの**Logs**タブに確認用URLがそのまま出力されるので、開発中の動作確認に使えます。
